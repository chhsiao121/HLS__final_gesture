...
localparam
    ...
    ADDR_ROWS_IN_DATA_0  = 7'h28,
    ADDR_ROWS_IN_CTRL    = 7'h2c,
    ADDR_COLS_IN_DATA_0  = 7'h30,
    ADDR_COLS_IN_CTRL    = 7'h34,
    ADDR_ROWS_OUT_DATA_0 = 7'h38,
    ADDR_ROWS_OUT_CTRL   = 7'h3c,
    ADDR_COLS_OUT_DATA_0 = 7'h40,
    ADDR_COLS_OUT_CTRL   = 7'h44,
    WRIDLE               = 2'd0,
    WRDATA               = 2'd1,
    WRRESP               = 2'd2,
    WRRESET              = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_rows_in = 'b0;
    reg  [31:0]                   int_cols_in = 'b0;
    reg  [31:0]                   int_rows_out = 'b0;
    reg  [31:0]                   int_cols_out = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_ROWS_IN_DATA_0: begin
                    rdata <= int_rows_in[31:0];
                end
                ADDR_COLS_IN_DATA_0: begin
                    rdata <= int_cols_in[31:0];
                end
                ADDR_ROWS_OUT_DATA_0: begin
                    rdata <= int_rows_out[31:0];
                end
                ADDR_COLS_OUT_DATA_0: begin
                    rdata <= int_cols_out[31:0];
                end
            endcase
        end
    end
...
assign rows_in   = int_rows_in;
assign cols_in   = int_cols_in;
assign rows_out  = int_rows_out;
assign cols_out  = int_cols_out;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_ROWS_IN_DATA_0)
            int_rows_in[31:0] <= (WDATA[31:0] & wmask) | (int_rows_in[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_COLS_IN_DATA_0)
            int_cols_in[31:0] <= (WDATA[31:0] & wmask) | (int_cols_in[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_ROWS_OUT_DATA_0)
            int_rows_out[31:0] <= (WDATA[31:0] & wmask) | (int_rows_out[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_COLS_OUT_DATA_0)
            int_cols_out[31:0] <= (WDATA[31:0] & wmask) | (int_cols_out[31:0] & ~wmask);
    end
end
